...
void ObjectFinder::RunProcess() {
	...
	for (int i = 0; i < 11; i++) {
		...
		std::ostringstream stringOfI;
		stringOfI << i;

		origImage = createImage(
					"/home/patbrant/Pictures/pren/street" + stringOfI.str()
							+ ".jpg");
		...
		cv::namedWindow("Street" + stringOfI.str(), cv::WINDOW_AUTOSIZE);
		imshow("Street" + stringOfI.str(), markedImage);
	}
	cv::waitKey(0);
}
